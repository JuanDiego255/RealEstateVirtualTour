                ...
                @foreach ($subcategories as $sub)
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card h-100 {{ !$sub->is_active ? 'border-secondary' : '' }}">
                            @if ($sub->image)
                                <img src="{{ route('file', ['filename' => $sub->image]) }}" class="card-img-top"
                                    style="height: 180px; object-fit: cover;" alt="{{ $sub->name }}">
                            @else
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex flex-wrap mt-2">
                                    @if ($pubCount > 0)
                                        <span class="badge badge-info mr-1 mb-1"><i
                                                class="fa fa-th-list mr-1"></i>{{ $pubCount }} Publicaciones</span>
                                    @else
                            ...
                            <div class="card-footer bg-white d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    <form action="{{ route('admin.subcategories.toggle-status', [$category, $sub]) }}"
                                        ...
                                        @csrf
                                        <button class="btn btn-{{ $sub->is_active ? 'warning' : 'success' }}"
                                            title="{{ $sub->is_active ? 'Desactivar' : 'Activar' }}">
                                            <i class="fa fa-{{ $sub->is_active ? 'eye-slash' : 'eye' }}"></i>
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.subcategories.destroy', [$category, $sub]) }}"
                                        method="POST" class="d-inline"
