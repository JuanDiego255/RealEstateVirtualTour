                ...
                <form class="form-horizontal" id="updateProperty{{ $property->id }}" action="{{ url('property/update/' . $property->id) }}" method="post"
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select class="form-control editPropertyType" name="property_type" data-id="{{ $property->id }}">
                            ...
                            <select class="form-control" name="subcategory_id">
                                ...
                                @if(isset($subcategories))
                                    @foreach ($subcategories as $sub)
                                        <option value="{{ $sub->id }}" {{ $property->subcategory_id == $sub->id ? 'selected' : '' }}>
                    ...
                    <div id="editFieldsRealEstate{{ $property->id }}" style="{{ ($property->property_type ?? '') == 'vehicle' ? 'display:none;' : '' }}">
                        ...
                        <div class="row">
                            <div class="form-group col-md-3">
                                ...
                                <input value="{{ $property->rooms }}" class="form-control" type="number" name="rooms" min="0">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Baños <span class="text-danger">*</span></label>
                                <input value="{{ $property->bathrooms }}" class="form-control" type="number" name="bathrooms" min="0">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Parqueos <span class="text-danger">*</span></label>
                                <input value="{{ $property->garage }}" class="form-control" type="number" name="garage" min="0">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Pisos <span class="text-danger">*</span></label>
                                <input value="{{ $property->floor_levels }}" class="form-control" type="number" name="floor_levels" min="0">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label>Construcción Mt² <span class="text-danger">*</span></label>
                                <input value="{{ $property->construction }}" class="form-control" type="number" name="construction" min="0">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Terreno Mt² <span class="text-danger">*</span></label>
                                <input value="{{ $property->land }}" class="form-control" type="number" name="land" min="0">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Año construcción <span class="text-danger">*</span></label>
                                <input value="{{ $property->construction_year }}" class="form-control" type="text" name="construction_year">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Mantenimiento <span class="text-danger">*</span></label>
                                <input value="{{ $property->maintenance }}" class="form-control" type="text" name="maintenance">
                            </div>
                        </div>
                    ...
                    {{-- ============ CAMPOS DE VEHÍCULO ============ --}}
                    <div id="editFieldsVehicle{{ $property->id }}" style="{{ ($property->property_type ?? '') != 'vehicle' ? 'display:none;' : '' }}">
                        <h6 class="text-muted mb-3 mt-3"><i class="fa fa-car"></i> Datos del Vehículo</h6>
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>Marca <span class="text-danger">*</span></label>
                                <input value="{{ $property->brand ?? '' }}" class="form-control" type="text" name="brand">
                            </div>
                            <div class="form-group col-md-4">
                                <label>Modelo <span class="text-danger">*</span></label>
                                <input value="{{ $property->model ?? '' }}" class="form-control" type="text" name="model">
                            </div>
                            <div class="form-group col-md-4">
                                <label>Año <span class="text-danger">*</span></label>
                                <input value="{{ $property->year ?? '' }}" class="form-control" type="text" name="year">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label>Color</label>
                                <input value="{{ $property->color ?? '' }}" class="form-control" type="text" name="color">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Kilometraje</label>
                                <input value="{{ $property->mileage_km ?? '' }}" class="form-control" type="text" name="mileage_km">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Tipo combustible</label>
                                <select class="form-control" name="fuel_type">
                                    <option value="">-- Seleccionar --</option>
                                    <option value="Gasolina" {{ ($property->fuel_type ?? '') == 'Gasolina' ? 'selected' : '' }}>Gasolina</option>
                                    <option value="Diésel" {{ ($property->fuel_type ?? '') == 'Diésel' ? 'selected' : '' }}>Diésel</option>
                                    <option value="Híbrido" {{ ($property->fuel_type ?? '') == 'Híbrido' ? 'selected' : '' }}>Híbrido</option>
                                    <option value="Eléctrico" {{ ($property->fuel_type ?? '') == 'Eléctrico' ? 'selected' : '' }}>Eléctrico</option>
                                    <option value="GLP" {{ ($property->fuel_type ?? '') == 'GLP' ? 'selected' : '' }}>GLP</option>
                                </select>
                            </div>
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control" name="transmission">
                                    <option value="">-- Seleccionar --</option>
                                    <option value="Automática" {{ ($property->transmission ?? '') == 'Automática' ? 'selected' : '' }}>Automática</option>
                                    <option value="Manual" {{ ($property->transmission ?? '') == 'Manual' ? 'selected' : '' }}>Manual</option>
                                    <option value="CVT" {{ ($property->transmission ?? '') == 'CVT' ? 'selected' : '' }}>CVT</option>
                                    <option value="Semiautomática" {{ ($property->transmission ?? '') == 'Semiautomática' ? 'selected' : '' }}>Semiautomática</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group col-md-3">
                                ...
                                <input value="{{ $property->engine_cc ?? '' }}" class="form-control" type="text" name="engine_cc">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Cap. tanque</label>
                                <input value="{{ $property->fuel_tank_capacity ?? '' }}" class="form-control" type="text" name="fuel_tank_capacity">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Tracción</label>
                                <select class="form-control" name="drivetrain">
                                    <option value="">-- Seleccionar --</option>
                                    <option value="4x2" {{ ($property->drivetrain ?? '') == '4x2' ? 'selected' : '' }}>4x2</option>
                                    <option value="4x4" {{ ($property->drivetrain ?? '') == '4x4' ? 'selected' : '' }}>4x4</option>
                                    <option value="AWD" {{ ($property->drivetrain ?? '') == 'AWD' ? 'selected' : '' }}>AWD</option>
                                    <option value="FWD" {{ ($property->drivetrain ?? '') == 'FWD' ? 'selected' : '' }}>FWD</option>
                                    <option value="RWD" {{ ($property->drivetrain ?? '') == 'RWD' ? 'selected' : '' }}>RWD</option>
                                </select>
                            </div>
                            <div class="form-group col-md-3">
                                <label>Llantas</label>
                                <input value="{{ $property->tires ?? '' }}" class="form-control" type="text" name="tires">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label>Puertas</label>
                                <input value="{{ $property->doors ?? '' }}" class="form-control" type="text" name="doors">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Pasajeros</label>
                                <input value="{{ $property->passengers ?? '' }}" class="form-control" type="text" name="passengers">
                            </div>
                            <div class="form-group col-md-3">
                                <label>Condición</label>
                                <select class="form-control" name="condition">
                                    <option value="">-- Seleccionar --</option>
                                    <option value="Nuevo" {{ ($property->condition ?? '') == 'Nuevo' ? 'selected' : '' }}>Nuevo</option>
                                    <option value="Usado" {{ ($property->condition ?? '') == 'Usado' ? 'selected' : '' }}>Usado</option>
                                </select>
                            </div>
                            <div class="form-group col-md-3">
                                <label>Placa</label>
                                <input value="{{ $property->plate ?? '' }}" class="form-control" type="text" name="plate">
                            </div>
                        </div>
                    </div>
...
(function() {
    ...
    function toggleEditFields{{ $property->id }}(type) {
        ...
        if (type === 'vehicle') {
            ...
            re.querySelectorAll('input, select').forEach(function(el) { el.removeAttribute('required'); });
        } else {
