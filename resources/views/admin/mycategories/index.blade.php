            ...
            @forelse($categories as $cat)
                <div class="col-md-6 col-lg-4 mb-4">
                    <div class="card h-100 {{ !$cat->is_active ? 'border-secondary' : '' }}">
                        @if($cat->cover_url)
                            <img src="{{ $cat->cover_url }}" class="card-img-top" style="height: 150px; object-fit: cover;">
                        @elseif($cat->image)
                            <img src="{{ route('file', $cat->image) }}" class="card-img-top" style="height: 150px; object-fit: cover;">
                        @else
                        ...
                        <div class="card-body">
                            ...
                            @if($cat->sector)
                                <span class="badge badge-info mb-2">
                                    @if($cat->sector->icon)<i class="{{ $cat->sector->icon }}"></i> @endif
                                    {{ $cat->sector->name }}
                                </span>
                            @endif
                        ...
                        <div class="card-footer d-flex justify-content-between">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('admin.categories.show', $cat) }}" class="btn btn-outline-info" title="Ver"><i class="fa fa-eye"></i></a>
                                <a href="{{ route('admin.categories.edit', $cat) }}" class="btn btn-outline-primary" title="Editar"><i class="fa fa-edit"></i></a>
                                <form action="{{ route('admin.categories.toggle-status', $cat) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button class="btn btn-outline-{{ $cat->is_active ? 'warning' : 'success' }}" title="{{ $cat->is_active ? 'Desactivar' : 'Activar' }}">
                                        <i class="fa fa-{{ $cat->is_active ? 'eye-slash' : 'eye' }}"></i>
                                    </button>
                                </form>
                            </div>
                            @if($cat->share_token)
                                <button class="btn btn-sm btn-outline-success" onclick="copyShareLink('{{ $cat->share_url }}')" title="Copiar enlace">
