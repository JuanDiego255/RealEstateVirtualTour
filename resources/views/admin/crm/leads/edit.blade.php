...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="{{ request('_back') ?: route('admin.crm.leads.show', $lead) }}" class="btn btn-secondary">
            ...
            <div class="card-body">
                <form action="{{ route('admin.crm.leads.update', $lead) }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="source" class="form-control" required>
                                            @foreach (\App\Lead::getSources() as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ old('source', $lead->source) == $key ? 'selected' : '' }}>
                                                    {{ $label }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="priority" class="form-control" required>
                                            @foreach (\App\Lead::getPriorities() as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ old('priority', $lead->priority) == $key ? 'selected' : '' }}>
                                                    {{ $label }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="interest_type" class="form-control" required>
                                            @foreach (\App\Lead::getInterestTypes() as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ old('interest_type', $lead->interest_type) == $key ? 'selected' : '' }}>
                                                    {{ $label }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                            ...
                                            <select name="user_id" class="form-control">
                                                ...
                                                @foreach ($agents as $agent)
                                                    <option value="{{ $agent->id }}"
                                                        {{ old('user_id', $lead->user_id) == $agent->id ? 'selected' : '' }}>
                                                        {{ $agent->name }}</option>
                                                @endforeach
                                            </select>
                                        @else
                                            <input type="hidden" name="user_id" value="{{ $lead->user_id }}">
                                            <p style="font-size:13px; color:#666;">Agente: <strong>{{ $lead->assignedTo->name ?? 'Sin asignar' }}</strong></p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <select name="property_id" class="form-control">
                                        ...
                                        @foreach ($properties as $property)
                                            <option value="{{ $property->id }}"
                                                {{ old('property_id', $lead->property_id) == $property->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="budget_currency" class="form-control">
                                            <option value="CRC"
                                                {{ old('budget_currency', $lead->budget_currency) == 'CRC' ? 'selected' : '' }}>
                                                CRC (₡)</option>
                                            <option value="USD"
                                                {{ old('budget_currency', $lead->budget_currency) == 'USD' ? 'selected' : '' }}>
                                                USD ($)</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div style="display:flex; gap:16px; margin-top:6px;">
                                    <label style="cursor:pointer; display:flex; align-items:center; gap:8px; font-size:14px;">
                                        <input type="radio" name="preferred_asset_type" value="property"
                                            {{ ($lead->preferred_asset_type ?? 'property') === 'property' ? 'checked' : '' }}
                                    ...
                                    <label style="cursor:pointer; display:flex; align-items:center; gap:8px; font-size:14px;">
                                        <input type="radio" name="preferred_asset_type" value="vehicle"
                                            {{ ($lead->preferred_asset_type ?? '') === 'vehicle' ? 'checked' : '' }}
                                            onchange="toggleAssetPrefs(this.value)"
                                            style="accent-color:#c2ac1f;">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="text" name="preferred_vehicle_brand" class="form-control"
                                                value="{{ old('preferred_vehicle_brand', $lead->preferred_vehicle_brand ?? '') }}"
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="number" name="preferred_vehicle_max_price" class="form-control"
                                                value="{{ old('preferred_vehicle_max_price', $lead->preferred_vehicle_max_price ?? '') }}"
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="number" name="preferred_vehicle_min_year" class="form-control"
                                                value="{{ old('preferred_vehicle_min_year', $lead->preferred_vehicle_min_year ?? '') }}"
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="number" name="preferred_vehicle_max_year" class="form-control"
                                                value="{{ old('preferred_vehicle_max_year', $lead->preferred_vehicle_max_year ?? '') }}"
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group mb-3">
                                            ...
                                            <select name="preferred_vehicle_fuel_type" class="form-control" style="border-radius:9px;">
                                                ...
                                                <option value="gasoline" {{ ($lead->preferred_vehicle_fuel_type ?? '') === 'gasoline' ? 'selected' : '' }}>Gasolina</option>
                                                <option value="diesel" {{ ($lead->preferred_vehicle_fuel_type ?? '') === 'diesel' ? 'selected' : '' }}>Diesel</option>
                                                <option value="hybrid" {{ ($lead->preferred_vehicle_fuel_type ?? '') === 'hybrid' ? 'selected' : '' }}>Híbrido</option>
                                                <option value="electric" {{ ($lead->preferred_vehicle_fuel_type ?? '') === 'electric' ? 'selected' : '' }}>Eléctrico</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group mb-3">
                                            ...
                                            <select name="preferred_vehicle_transmission" class="form-control" style="border-radius:9px;">
                                                <option value="">— Cualquiera —</option>
                                                <option value="automatic" {{ ($lead->preferred_vehicle_transmission ?? '') === 'automatic' ? 'selected' : '' }}>Automática</option>
                                                <option value="manual" {{ ($lead->preferred_vehicle_transmission ?? '') === 'manual' ? 'selected' : '' }}>Manual</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div>
                            <a href="{{ request('_back') ?: route('admin.crm.leads.show', $lead) }}"
                ...
                <form id="delete-form" action="{{ route('admin.crm.leads.destroy', $lead) }}" method="POST"
                    style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
