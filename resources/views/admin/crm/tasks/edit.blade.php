    ...
    <div class="crm-page-header">
        <div>
            ...
            <div class="sub">
                ...
                &nbsp;·&nbsp;
                <span class="crm-badge {{ $task->priority }}">{{ \App\LeadTask::getPriorities()[$task->priority] ?? $task->priority }}</span>
            </div>
        </div>
        <div class="actions">
                ...
                <div class="dashboard-card">
                    ...
                    <div class="dc-body">
                                ...
                                <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:12px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="margin-top:3px;">
                                            ...
                                            @if($task->lead->phone)
                                                <span style="font-size:11px;color:#888;margin-left:8px;"><i class="fa fa-phone"></i> {{ $task->lead->phone }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="assigned_to" class="form-control @error('assigned_to') is-invalid @enderror">
                                ...
                                @foreach($agents as $agent)
                                    <option value="{{ $agent->id }}" {{ old('assigned_to', $task->assigned_to) == $agent->id ? 'selected' : '' }}>
                                        {{ $agent->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if($task->status === 'completed')
                            <div class="form-group" style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 16px;">
                                ...
                                <textarea name="completion_notes" rows="2"
                                          class="form-control"
                                          style="background:#fff;border-color:#bbf7d0;"
                                ...
                                @if($task->completed_at)
                                    <small style="color:#22c55e;font-size:11px;margin-top:4px;display:block;">
                                        <i class="fa fa-clock-o"></i> Completada el {{ $task->completed_at->format('d/m/Y \a \l\a\s H:i') }}
                                    </small>
                                @endif
                            </div>
                        @endif
        ...
        <div class="col-lg-4">
            ...
            <div class="dashboard-card" style="margin-bottom:18px;">
                ...
                <div class="dc-body" style="padding:14px 16px;">
                    <div style="display:flex;flex-direction:column;gap:10px;font-size:13px;">
                        <div>
                            <div style="font-size:11px;color:#888;text-transform:uppercase;font-weight:600;margin-bottom:2px;">Creada por</div>
                            <div style="color:#1a1a2e;">{{ $task->creator->name ?? '—' }}</div>
                        </div>
                        <div>
                            <div style="font-size:11px;color:#888;text-transform:uppercase;font-weight:600;margin-bottom:2px;">Fecha de creación</div>
                            <div style="color:#1a1a2e;">{{ $task->created_at->format('d/m/Y H:i') }}</div>
                        </div>
                        @if($task->due_at)
                            <div>
                                <div style="font-size:11px;color:#888;text-transform:uppercase;font-weight:600;margin-bottom:2px;">Fecha límite</div>
                                @php
                        ...
                        @if($task->assignee)
                            <div>
                                <div style="font-size:11px;color:#888;text-transform:uppercase;font-weight:600;margin-bottom:2px;">Asignado a</div>
                                <div style="color:#1a1a2e;display:flex;align-items:center;gap:6px;">
                                    <div style="width:24px;height:24px;border-radius:50%;background:#1a1a2e;color:#c2ac1f;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;">
            ...
            <div class="dashboard-card" style="margin-bottom:18px;">
                ...
                <div class="dc-body" style="padding:10px 12px;display:flex;flex-direction:column;gap:4px;">
                    @if($task->lead)
                        <a href="{{ route('admin.crm.leads.show', $task->lead) }}" class="action-btn secondary" style="justify-content:flex-start;">
                            <i class="fa fa-user"></i> Ver lead
                        </a>
                        <a href="{{ route('admin.crm.tasks.create', ['lead_id' => $task->lead_id]) }}" class="action-btn secondary" style="justify-content:flex-start;">
                            <i class="fa fa-plus"></i> Nueva tarea para este lead
                        </a>
                    @endif
                    <a href="{{ route('admin.crm.tasks.index') }}" class="action-btn secondary" style="justify-content:flex-start;">
            ...
            <div class="dashboard-card">
                ...
                <div class="dc-body">
                    <p style="font-size:13px;color:#666;margin-bottom:12px;">Eliminar esta tarea de forma permanente.</p>
                ...
                <div class="modal-body" style="padding:18px;">
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <textarea name="completion_notes" rows="3" class="form-control"
                                  placeholder="Resultado, observaciones..."></textarea>
                    </div>
                </div>
